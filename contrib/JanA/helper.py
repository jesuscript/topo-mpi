...
def package_images(packages=1,randomize=False):
    import Image
    import subprocess
    index = numpy.arange(0,900*packages,1)
    #image_filenames=["./Mice/Stimuli/NG/tifsequence/frame%05d.tif" %(i) for i in index]
    image_filenames=["./Mice/Stimuli/SparseNoise_DS=6.0_Step=10_Density=3/up_frame%05d.tif" %(i) for i in index]
	...
	for j in xrange(0,900):
	    command.append(image_filenames[i*900+j])		
	#command.append("./Mice/Stimuli/NG/NIFliInvStack"+str(i)+".tif")
	command.append("./Mice/Stimuli/SparseNoise_DS=6.0_Step=10_Density=3/SparseNoiseStack"+str(i)+".tif")
...
def generateSparseNoiseStimuli(square_in_deg,steps,num_inputs,density):
	...
	framefile = "./Mice/Stimuli/SparseNoise_DS=6.0_Step=10_Density=3/frame%05d.tif" % i
	image.save(framefile)	
	framefile = "./Mice/Stimuli/SparseNoise_DS=6.0_Step=10_Density=3/up_frame%05d.tif" % i
