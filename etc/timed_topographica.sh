...
/afs/inf.ed.ac.uk/user/v/v1kkudr2/topographica/bin/mpiexec -n $NCPU --hostfile hostfile --bynode   topographica --mpi -p cortex_density=$CORTEX -p retina_density=$RETINA -p lgn_density=$RETINA -p mpi=True $SCRIPT -c "import timeit; print 'Parallel:', timeit.Timer('topo.sim.run($ITERATIONS)','import topo').timeit(number=1)" -c "import numpy; numpy.savetxt('topo_parallel_activity.out',topo.sim['V1'].activity)"
